...
def load_pretrained(model: str, metric: str, quality: int, config_path=None) -> nn.Module:
    return pretrained_models[model](
        quality=quality, metric=metric, pretrained=True, progress=False
    ).eval()


def load_checkpoint(arch: str, no_update: bool, checkpoint_path: str, config_path=None) -> nn.Module:
    ...
    elif arch in model_mine:
        model_cls = model_mine[arch]
...
@torch.no_grad()
def inference_patch_fold(model, x, patch_size, vbr_stage=None, vbr_scale=None):
    x = x.unsqueeze(0)
    ...
    start = time.time()
    out_enc = (
        model.compress_patch(x_padded, patch_size)
        if vbr_scale is None
        else model.compress(x_padded, stage=vbr_stage, s=0, inputscale=vbr_scale)
    )
    enc_time = time.time() - start
    ...
    start = time.time()
    out_dec = (
        model.decompress_patch(out_enc["strings"], out_enc["shape"], patch_size)
        if vbr_scale is None
        else model.decompress(
            out_enc["strings"],
            out_enc["shape"],
    ...
    for filepath in tqdm(filepaths):
        ...
        if not entropy_estimation:
            ...
            if not args["patch"]:
                ...
                if inputdir == 'Kodak':
                    patch_size = 2 ** 7
                elif inputdir == 'DIV8k' or inputdir == 'CLIC':
                    patch_size = 2 ** 8

                rv, output_image = (
                    inference_patch_fold(model, x, patch_size)
                    if not is_vbr_model
                    else inference(model, x, vbr_stage, vbr_scale)
                )
        ...
        if args["per_image"]:
            ...
            if args["save_image"]:
                compressed_path = output_subdir / "images"
                compressed_path.mkdir(parents=True, exist_ok=True)
                compressed_image_path = compressed_path / f"{filepath.stem}.png"
                rec_net = transforms.ToPILImage()(output_image.squeeze().cpu())
                rec_net.save(compressed_image_path)
            
            output_subdir = output_subdir / "metric"
            output_subdir.mkdir(parents=True, exist_ok=True)
...
def setup_args():
    ...
    parent_parser.add_argument("--dataset", type=str, default='Kodak', help="dataset path")
...
def main(argv):  # noqa: C901
    ...
    filepaths = collect_images(DATASET_PATH[args.dataset])
    if len(filepaths) == 0:
    ...
    for run in runs:
        if args.verbose:
            ...
            sys.stderr.flush()
        if not is_vbr_model:
            model = load_func(*opts, run, config_path=args.config)
        else:
